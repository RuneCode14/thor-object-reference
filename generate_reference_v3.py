...
def generate_markdown(type_name: str, schema: dict) -> str:
    ...
    for field_name in sorted(flattened_props.keys()):
        ...
        if nested_flat:
            ...
            doc.append("")
            doc.append(f"> ⚠️ **These nested fields are JSON structure reference only.** THOR's Sigma backend matches on **top-level fields only**. You cannot use `IMAGE.PATH`, `IMAGE_PATH`, or `PARENT_INFO.PID` in Sigma rules.")
            doc.append(f"> Object null-check syntax (`FIELD: null`) exists but matched **all objects** in THOR v11.0.0 testing — verify behavior before relying on it.")
            doc.append("")
...
def generate_summary(json_data: dict, output_dir: Path) -> str:
    ...
    doc.append("- Object null-check syntax exists but was observed matching all objects in THOR v11.0.0 — **verify before relying on it**")
    doc.append("")
